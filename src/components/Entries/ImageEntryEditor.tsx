...
interface ImageEntryEditorProps {
  isEditing?: boolean;
}

const ImageEntryEditor: React.FC<ImageEntryEditorProps> = ({ isEditing = false }) => {
  const { entryId } = useParams();
  const navigate = useNavigate();
  const { currentUser } = useAuth();
  
  // State variables
  const [title, setTitle] = useState('New Image');
  ...
  const categoryOptions = [
    'Personal', 'Nature', 'Scripture', 'Church',
    'People', 'Travel', 'Events', 'Inspiration',
    'Teaching', 'Biblical Scenes', 'Art', 'History',
    'Worship', 'Biblical Sites', 'Ministry', 'Illustrations'
  ];
      ...
      if (isEditing && entryId && currentUser) {
        try {
          ...
          if (loadedEntry && loadedEntry.userId === currentUser.uid) {
            setTitle(loadedEntry.title || 'New Image');
  ...
  const uploadImage = async (imageId: string): Promise<string | null> => {
    ...
    try {
      ...
      const storageRef = ref(storage, `images/${currentUser?.uid}/${imageId}`);
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full min-h-[100px] rounded-md border border-gray-300 px-3 py-2 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
...
export default ImageEntryEditor; 